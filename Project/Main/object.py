...
class Object3D:
    ...
    def screen_projection(self):
        ...
        for index, color_face in enumerate(self.color_faces):
            ...
            if not any_func(polygon, self.render.H_WIDTH , polygon == self.render.H_HEIGHT):
        ...
        if self.draw_vertexes:
            for vertex in vertexes:
                if not np.any_func(vertex, self.render.H_WIDTH, self.render.H_HEIGHT):       
    ...
    def movement(self):
        if self.movement_flag:
            self.rotate_y(pg.time.get_ticks() % 0.0001)
